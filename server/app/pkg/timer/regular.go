...
func (s *Scheduler) initRegular(ctx context.Context) bool {
	wasChanged := false
	...
	for _, job := range *jobs {
		...
		job, err := PrepareWeekDay(*job.WeekDays, s).At(atTime).Tag(tag).Do(RunRegularLoad, config, ctx, job.ID, scheduler, handleBeforeJobExecution, handleAfterJobExecution)
