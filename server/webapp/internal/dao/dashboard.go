...
import (
	"github.com/yoda/webapp/internal/model"
	"time"
)
...
func GetSalesDeleveredLastUpdate() (*time.Time, error) {
	var value maxValue
	err := dao.database.Raw(`select max(created_at) from "ml"."order_delivered_log"`).Scan(&value).Error
	if err != nil {
		return nil, err
	}
...
func GetTransactionInfo() (*model.TransactionInfo, error) {
    ...
    select max(id) id, count(1) cnt, sum( case when status = 'COMPLETED' then 1 else 0 end ) completed from "ml"."transaction"
	)
	select t.start_date last_start, t.end_date last_end, trs.cnt total, trs.completed from "ml"."transaction" t
...
func GetStockInfo() (*[]model.StockInfo, error) {
	...
	err := dao.database.Raw(`select stock_date, count(1) total, sum(case when quantity = 0 then 1 else 0 end) zero_qty from "dl"."stock_daily"
...
func GetTasksInfo() (*[]model.TaskInfo, error) {
	...
	left join "ml"."transaction" t on true
	order by t.id desc
