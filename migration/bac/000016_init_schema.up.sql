create table dl.report_by_cluster
(
    report_date      date,
    source           varchar(5),
...
CREATE TABLE "dl"."report_by_cluster_def" PARTITION OF "dl"."report_by_cluster" DEFAULT;

create
or replace procedure "dl".partition_for_report_by_cluster(IN start_date date, IN end_date date)
    language plpgsql
as
$$
declare
...
call "dl".partition_for_report_by_cluster(to_date('2023-01-01', 'yyyy-MM-dd'), to_date('2024-01-01', 'yyyy-MM-dd'));

create procedure calc_report_by_cluster(IN p_day date)
    language plpgsql
as
$$
declare
...
insert into dl.report_by_cluster(report_date, source, owner_code, supplier_article, barcode, external_code, def30, days_in_stock30, def5, days_in_stock5, avg_price, min_price, max_price, quantity30,
